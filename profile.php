...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($email !== $_SESSION['user']['email']) {
            ...
            $stmt->execute([':email' => $email, ':id' => $teacher_id]);
            if ($stmt->fetch()) {
        ...
        if (empty($errors)) {
            ...
            if (!empty($new_password)) {
                ...
                $stmt = $db->prepare("UPDATE users SET full_name = :full_name, email = :email, 
                                    password = :password, academic_title = :academic_title, 
                ...
                $stmt = $db->prepare("UPDATE users SET full_name = :full_name, email = :email, 
                                    academic_title = :academic_title, department = :department 
                                    WHERE id = :id");
                $stmt->execute([
                    ':full_name' => $full_name,
                    ':email' => $email,
            ...
            $_SESSION['user']['academic_title'] = $academic_title;
...
<head>
    ...
    <title>Profil Ayarları | CoreLab Akademik Yönetim</title>
    ...
    <div class="main-content">
        ...
        <div class="profile-container">
            ...
            <div class="profile-card">
                ...
                <h2 class="profile-name"><?= htmlspecialchars($teacher['full_name'], ENT_QUOTES, 'UTF-8') ?></h2>
                <div class="profile-title"><?= $academicTitles[$teacher['academic_title'] ?? 'Akademisyen'] ?? 'Akademisyen' ?></div>
                
                <?php if (!empty($teacher['department'])): ?>
                    <div class="profile-department">
                        <i class="fas fa-university"></i>
                        <span><?= htmlspecialchars($teacher['department'], ENT_QUOTES, 'UTF-8') ?></span>
                    </div>
                <?php endif; ?>
                
                <div class="profile-stats">
                    <div class="stat-item">
                        <div class="stat-value"><?= date('Y') - date('Y', strtotime($teacher['created_at'])) ?></div>
                        <div class="stat-label">Yıllık Üyelik</div>
                    </div>
                    <div class="stat-item">
                        <div class="stat-value"><?= strtoupper(substr($teacher['role'], 0, 1)) ?></div>
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            ...
                            <input type="email" id="email" name="email" class="form-control" 
                                   value="<?= htmlspecialchars($teacher['email'], ENT_QUOTES, 'UTF-8') ?>" required>
                        </div>
                    </div>
                    
                    <div class="form-grid">
                        <div class="form-group">
                            <label for="academic_title">Akademik Unvan</label>
                            <select id="academic_title" name="academic_title" class="form-control" required>
                                <option value="">Seçiniz</option>
                                <option value="prof" <?= $teacher['academic_title'] === 'prof' ? 'selected' : '' ?>>Prof. Dr.</option>
                                <option value="assoc_prof" <?= $teacher['academic_title'] === 'assoc_prof' ? 'selected' : '' ?>>Doç. Dr.</option>
                                <option value="assist_prof" <?= $teacher['academic_title'] === 'assist_prof' ? 'selected' : '' ?>>Dr. Öğr. Üyesi</option>
                                <option value="lecturer" <?= $teacher['academic_title'] === 'lecturer' ? 'selected' : '' ?>>Öğr. Gör.</option>
                                <option value="res_assist" <?= $teacher['academic_title'] === 'res_assist' ? 'selected' : '' ?>>Arş. Gör.</option>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="department" name="department" class="form-control" 
                                   value="<?= htmlspecialchars($teacher['department'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
                        </div>
                    </div>
